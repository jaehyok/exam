...
public class CD572 {
/*
 * http://codingdojang.com/scode/572
 ...
 */
	...
	exec(cacheSize1, cities1);
	exec(cacheSize2, cities2);
	exec(cacheSize3, cities3);
	exec(cacheSize4, cities4);
	exec(cacheSize5, cities5);
	exec(cacheSize6, cities6);
    }

    private void exec(int cacheSize, String[] cities) {
	int result = 0;
	Map<String, Integer> cache = new HashMap<>();
	for (String city : cities) {
	    if(hitCache(cache, cacheSize, city.toLowerCase()))
		result += timeHit;
	    else
		result += timeMiss;
	}
    ...
    private boolean hitCache(Map<String, Integer> cache, int cacheSize, String city) {
	int order = cache.getOrDefault(city, -1);
	if (order == -1) {
	    addCache(cache, cacheSize, city);
	    return false;
	} else {
	    orderCache(cache, city);
	    return true;
	}
    }

    private void orderCache(Map<String, Integer> cache, String city) {
    ...
    private void addCache(Map<String, Integer> cache, int cacheSize, String city) {
	if(cache.size() >= cacheSize) {
	    removeOld(cache);
	}
	cache.put(city, cache.size() + 1);
    }
